...
sequenceDiagram
    ...
    Prospect->>QuoteSystem: Request Pricing (Public Form)
    QuoteSystem->>SalesAgent: Notify Lead
    SalesAgent->>QuoteSystem: Build & Send Quote
    QuoteSystem->>Prospect: Email Quote Link
    Prospect->>QuoteSystem: View & Negotiate
    Prospect->>QuoteSystem: Sign & Accept
    QuoteSystem->>Finance: Convert to Invoice
    Finance->>Prospect: Send Invoice / Charge Card
    QuoteSystem->>SalesAgent: Notify Deal Won
```
